...
public class BlockRoot : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(!this.is_has_falling_block()){
            ...
            do {
                ...
                //슬라이드 할 곳의 블록이 잡을 수 있는 상태가 아니라면
                ...
                if(offset < Block.COLLSION_SIZE / 2.0f) {
                    break;                  //루프 탈출
                }
                //블록을 교체한다
                this.swapBlock(grabbed_block, grabbed_block.slide_dir, swap_target);

                this.grabbed_block = null; //지금은 블록을 잡고있지 않다
            } while(false);
    ...
    //현재 패턴의 출현 확률을 바탕으로 색을 산출해서 반환한다
    public Block.COLOR selectBlockColor() {
        ...
        float rand = UnityEngine.Random.Range(0.0f, 1.0f);              //0.0~1.0사이의 난수
    ...
    //지정된 그리드 좌표의 열에 슬라이드 중인 블록이 하나라도 있으면 true
    ...
    public void initalSetup() {
        ...
        for(int y = 0; y < Block.BLOCK_NUM_Y; y++) {            //처음부터 마지막행까지
            for(int x = 0; x <Block.BLOCK_NUM_X; x++) {         //왼쪽부터 오른쪽까지
                ...
                color_index = 
                    UnityEngine.Random.Range(0, (int)Block.COLOR.NORMAL_COLOR_NUM);
    ...
    public static Vector3 calcBlockPosition(Block.iPosition i_pos)
    {
        ...
        //초깃값 + 그리드 좌표 x 블록 크기
        position.x += (float)i_pos.x * Block.COLLSION_SIZE;
        position.y += (float)i_pos.y * Block.COLLSION_SIZE;
    ...
    public bool unprojectMousePosition(out Vector3 world_position, Vector3 mouse_position)
    {
       bool ret;
        ...
        //블록의 절반 크기만큼 앖에 둔다
       Plane plane = new Plane(Vector3.back, new Vector3(0.0f, 0.0f, -Block.COLLSION_SIZE / 2.0f));
    ...
    public static Vector3 getDirVector(Block.DIR4 dir) {
        ...
        v *= Block.COLLSION_SIZE;                                   //블록의 크기를 곱한다.
    ...
    public static Block.DIR4 getOppositeDir(Block.DIR4 dir) {
        ...
        switch(dir) {
            case Block.DIR4.RIGHT : opposite = Block.DIR4.LEFT; break;
            case Block.DIR4.LEFT : opposite = Block.DIR4.RIGHT; break;
            case Block.DIR4.UP : opposite = Block.DIR4.DOWN; break;
            case Block.DIR4.DOWN : opposite = Block.DIR4.UP; break;
        }
    ...
    public void swapBlock(BlockControl block0, Block.DIR4 dir, BlockControl block1) {
        ...
        block0.beginSlide(offset1);                 //이동할 위치의 블록 이동을 시작한다
    }
}
