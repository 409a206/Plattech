...
public class Block {
    public static float COLLSION_SIZE = 1.0f;       //블록의 충돌 크기
...
public class BlockControl : MonoBehaviour
{
    ...
    private Vector3 position_offset_inital = Vector3.zero;      //교체 전 위치
    ...
    void Update()
    {
        ...
        block_root.unprojectMousePosition( 
            out mouse_position, Input.mousePosition);           //마우스 위치 획득
        ...
        if(this.next_step == Block.STEP.NONE) {                 //상태 정보 없음의 경우
            switch(this.step) {
        ...
        while(this.next_step != Block.STEP.NONE) {
            ...
            switch(this.step) {
                ...
                case Block.STEP.RESPAWN :
                    ...
                    int color_index = UnityEngine.Random.Range(
        ...
        switch(this.step) {
            ...
            //(어려운 부분이니 지금은 몰라도 괜찮다)
                ...
                this.position_offset = Vector3.Lerp(
                    this.position_offset_inital, Vector3.zero, rate
                );
                break;
            case Block.STEP.FALL : 
                //속도에 중력의 영향을 부여한다
    ...
    public bool isVisible() {
        //그리기 기능(renderer.enabled가 true) 상태라면
        //표시되고 있다
    ...
    //자신이 잡을 수 있는 상태인지 반환한다.
    public bool isGrabbable() {
        bool is_grabbable = false;
        switch(this.step) {
            case Block.STEP.IDLE :                      //'대기' 상태 일때만
    ...
    //지정된 마우스 좌표가 자신에게 겹치는지 반환한다.
    public bool isContainedPosition(Vector2 position) {
        bool ret = false;
        Vector3 center = this.transform.position;
        float h = Block.COLLSION_SIZE / 2.0f;
        ...
        do {
            ...
            //x 좌표, y 좌표 모두 겹쳐 있으면 true(겹쳐있다)를 반환한다.
            ret = true;
        } while(false);
        return ret;
    }

    //마우스 위치를 바탕으로 슬라이드 된 방향을 구한다
    ...
    //블록을 교체하기 시작한다. 이동 시작을 알리는 메소드
    public void beginSlide(Vector3 offset) {
        this.position_offset_inital = offset;
        this.position_offset = this.position_offset_inital;
        //상태를 SLIDE로 변경
    ...
    public void beginFall(BlockControl start) {
        ...
        this.position_offset.y = 
            (float)(start.i_pos.y - this.i_pos.y) * Block.COLLSION_SIZE;
    }
    public void beginRespawn(int start_ipos_y) {
        ...
        this.next_step = Block.STEP.FALL;
        // int color_index = 
        //     UnityEngine.Random.Range((int) Block.COLOR.FIRST,
